...
//==============================================================================
// Step 2: Coordinate Transformation (Quest -> Robot EEF)
//==============================================================================

// Transform Quest controller data to robot EEF frame
// Quest: front=+Z, up=+Y, right=+X
// Robot: front=+Z, up=+X, right=+Y
// Mapping: Quest (x, y, z) -> Robot (y, x, z)
//          Quest X (right) -> Robot Y (right)
//          Quest Y (up)    -> Robot X (up)
//          Quest Z (front) -> Robot Z (front)
ControllerData transformToRobotFrame(const ControllerData& quest_data) {
  ...
  // Quaternion transformation (same axis swap)
...
public:
  ...
  {
    // Publisher for LEFT arm trajectory only
  ...
  void sendHomingTrajectory() {
    std::vector<std::string> joint_names;
    std::vector<double> positions;

    for (int i = 1; i <= 7; ++i) {
      std::string joint_name = "openarm_left_joint" + std::to_string(i);
    ...
    RCLCPP_INFO(node_->get_logger(), "[HOMING] left arm: joint4 -> 1.58 (4s)");
  }

  // Check if LEFT joint4 reached target
  bool isHomingComplete() {
    double current = getJointPosition("openarm_left_joint4");
    return std::abs(current - 1.58) < 0.03;  // Tolerance ±0.03 rad
  }

  // Execute homing for LEFT arm only
  bool executeHoming() {
    ...
    sendHomingTrajectory();

    rclcpp::Rate rate(10);
    while (rclcpp::ok() && !isHomingComplete()) {
      rclcpp::spin_some(node_);
      rate.sleep();
    }
...
public:
  ...
  void sendGripperCommand(double trigger_value) {
    ...
    if (gripper_client_left_->action_server_is_ready()) {
      gripper_client_left_->async_send_goal(goal_msg, send_goal_options);
    }
  }

  // Update gripper based on Quest trigger value (LEFT only)
  void update(double left_trigger) {
    // Only send if trigger changed significantly
    if (std::abs(left_trigger - prev_trigger_left_) > 0.01) {
      sendGripperCommand(left_trigger);
      prev_trigger_left_ = left_trigger;
    }
  }

  // Open gripper (for homing)
  void open() {
    sendGripperCommand(0.0);   // trigger 0 = open
...
//==============================================================================
// Step 6: Left Arm Teleop Controller
//==============================================================================

class LeftArmTeleop {
public:
  LeftArmTeleop(rclcpp::Node::SharedPtr node)
    : node_(node),
      calibrator_left_(1.0),   // 1 second calibration
      prev_timestamp_left_(0.0),
      left_calibrated_(false)
  {
    RCLCPP_INFO(node_->get_logger(), "[LeftArmTeleop] Creating LEFT servo...");

    // Create LEFT ServoController
    servo_left_ = std::make_unique<ServoController>(
        node_, "moveit_servo_left", "openarm_left_link0");

    RCLCPP_INFO(node_->get_logger(), "[LeftArmTeleop] Left servo created!");
  }
  ...
  // Process Quest data and send to LEFT servo only
  void update(const QuestData& robot_data) {
    // Process left arm only
...
int main(int argc, char* argv[])
{
  ...
  RCLCPP_INFO(node->get_logger(), "=== Quest VR Teleop for OpenArm (LEFT ARM ONLY) ===");
  ...
  RCLCPP_INFO(node->get_logger(), "Step 3: Calibration (1 sec)");
  RCLCPP_INFO(node->get_logger(), "Step 4: Velocity Conversion");
  RCLCPP_INFO(node->get_logger(), "Step 5: Servo API Integration");
  RCLCPP_INFO(node->get_logger(), "Step 6: Left Arm Teleop");
  RCLCPP_INFO(node->get_logger(), "Step 7: Left Gripper Control");

  // Wait for system to be ready
  RCLCPP_INFO(node->get_logger(), "Waiting 3 seconds for system ready...");
  std::this_thread::sleep_for(std::chrono::seconds(3));

  // Create LEFT arm teleop controller
  LeftArmTeleop teleop(node);
  if (!teleop.isReady()) {
    RCLCPP_ERROR(node->get_logger(), "Failed to initialize left servo controller!");
    return 1;
  }

  // Start socket server for Quest VR input
  ...
  // Create gripper controller and open LEFT gripper after homing
  GripperController gripper(node);
  gripper.open();
  RCLCPP_INFO(node->get_logger(), "[GRIPPER] Opening left gripper...");
  ...
  while (rclcpp::ok()) {
    ...
    // Get Quest data from socket server
    QuestData quest_raw = socket_server.getLatestData();

    // Transform to robot frame (Quest Y-up -> Robot Z-up)
    QuestData robot_data = transformQuestData(quest_raw);

    // Process teleop
    teleop.update(robot_data);

    // Step 7: Update LEFT gripper based on trigger value
    if (robot_data.left.enabled) {
      gripper.update(robot_data.left.trigger);
    }

    // Log every second (LEFT arm only)
    if (++log_counter % 100 == 0) {
      RCLCPP_INFO(node->get_logger(),
        "Socket: %s | LEFT: cal=%d en=%d pos=(%.3f,%.3f,%.3f) trig=%.2f",
