...
int main(int argc, char* argv[])
{
  ...
  RCLCPP_INFO(node->get_logger(), "Waiting for Quest connection...");
  ...
  // Step 8: Safety - connection timeout
  ...
  while (rclcpp::ok()) {
    ...
    bool is_connected = socket_server.isConnected();

    // Log connection status change
    if (is_connected && !was_connected) {
      RCLCPP_INFO(node->get_logger(), "Quest connected! Hold controllers steady for calibration...");
    } else if (!is_connected && was_connected) {
      RCLCPP_INFO(node->get_logger(), "Quest disconnected.");
    }
    was_connected = is_connected;

    if (is_connected) {
      // Get raw Quest data and transform to robot frame
      QuestData quest_raw = socket_server.getLatestData();
      QuestData robot_data = transformQuestData(quest_raw);

      if (robot_data.timestamp > last_data_time) {
        last_data_time = robot_data.timestamp;
